...
namespace NCAA_Data
{
	public class Kicking : Base
	{
        ...
        public override void GetParameters()
        {
            ...
            Console.WriteLine("Which kicking stat would you like to see: FGM, FGA, XPM, XPA, PCT, PTS");
            StatType = Console.ReadLine().ToUpper();
        }

        public override async Task DisplayStatistics()
        {
            string selectedCategory = "kicking";
            ...
            if (sortOrder == "asc")
            {
                filteredStats.Sort((ps1, ps2) => double.Parse(ps1.Stat).CompareTo(double.Parse(ps2.Stat)));
            }
            else if (sortOrder == "desc")
            {
                filteredStats.Sort((ps1, ps2) => double.Parse(ps2.Stat).CompareTo(double.Parse(ps1.Stat)));
            }
            else
            {
